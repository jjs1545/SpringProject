		        ...
		        <table class="table table-bordered">
		         	...
		         	<tbody id="tbody">
			        	<c:forEach items="${list }" var="list" varStatus="status">
			         		<input type="hidden" id="postCount" value="${list.post_count }">
			         		<tr>
			         			<td>${list.no }</td>
			         			<td>${list.name }</td>
			         			<td><span id="postCount">${list.post_count }</span></td>
			         			<td>${list.description }</td>
			         			<%-- <a href="javascript:getData('${list.no }')" style="color:black"> --%>
			         			<c:choose>
			         			<c:when test="${list.post_count == 0 }">
			...
			$.ajax({
				...
				success: function(result) {
					...
					for(var i=0; i<result.data.length; i++) {
						...
						if(result.data[i].post_count == 0 ){
							temp += '<td><button id="deleteBtn" class="form-control" value="'+result.data[i].no+'">X</button></td>';			
						}
						else{
							temp += '<td>&nbsp;</td>'
						}			
